...
sub _execute {
    ...
    if ( $self->dump_scenario ) {
        ...
        my $orig_error_level = Treex::Core::Log::get_error_level();
		Treex::Core::Log::log_set_error_level('FATAL');
        my $dummy_scenario = Treex::Core::Scenario->new( { from_string => 'Util::SetGlobal hack="yes"' } );
        Treex::Core::Log::log_set_error_level($orig_error_level);
