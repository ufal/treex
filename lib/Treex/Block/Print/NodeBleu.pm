package Treex::Block::Print::NodeBleu;
use Moose;
...
has attributes => ( isa=>'Str', is=>'ro', required=>1, documentation => 'Space or comma separated list of attributes' );
...
sub process_bundle {
    my ( $self, $bundle ) = @_;
    my $tst_tree = $bundle->get_zone( $self->language, $self->selector )->get_tree($self->layer);
    my $ref_tree = $bundle->get_zone( $self->language, $self->reference_selector )->get_tree($self->layer);
    my @attr_names = split /[, ]/, $self->attributes;
    my $tst = join ' ', map {join 'A', $_->get_attrs(@attr_names)} $tst_tree->get_descendants({ordered=>1});
    my $ref = join ' ', map {join 'A', $_->get_attrs(@attr_names)} $ref_tree->get_descendants({ordered=>1});
    Treex::Tool::Eval::Bleu::add_segment( $tst, $ref, 1 );
    return;
}
