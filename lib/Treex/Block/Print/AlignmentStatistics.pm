...
sub print_alignment_info {
	...
	if ($self->one_to_one) {
		...
		foreach my $i (0..$#one_to_one_s) {
			...
			my $out_string = sprintf("%30s\t%40s\t%20s\t%20s\t%4d\t%4d", $sid, $tid, $one_to_one_s[$i]->form, $one_to_one_t[$i]->form, $one_to_one_s[$i]->ord, $one_to_one_t[$i]->ord);
	...
	if ($self->one_to_many) {
		...
		foreach my $i (0..$#one_to_many_s) {
			...
			foreach my $j (0..$#tmp_one_many_t) {
				my $out_string;
				if (($i == 0) && ($j == 0)) {
					$out_string = sprintf("%30s\t%40s\t%-20s\t%-20s\t%-4d\t%-4d", $sid, $tid, $one_to_many_s[$i]->form, $tmp_one_many_t[$j]->form, $one_to_many_s[$i]->ord, $tmp_one_many_t[$j]->ord);					
				}
				elsif (($i != 0) && ($j == 0)) {
					$out_string = sprintf("%30s\t%40s\t%-20s\t%-20s\t%-4d\t%-4d", '', '', $one_to_many_s[$i]->form, $tmp_one_many_t[$j]->form, $one_to_many_s[$i]->ord, $tmp_one_many_t[$j]->ord);
				}
				else {
					$out_string = sprintf("%30s\t%40s\t%-20s\t%-20s\t%-4s\t%-4d", '', '', '', $tmp_one_many_t[$j]->form, '', $tmp_one_many_t[$j]->ord);
				}
				print { $self->_file_handle } $out_string . "\n";				
			}
		}		
	...
	if ($self->many_to_one) {
		...
		foreach my $i (0..$#many_to_one_s) {
			...
			foreach my $j (0..$#tmp_many_one_s) {
				my $out_string;
				if (($i == 0) && ($j == 0)) {
					$out_string = sprintf("%30s\t%40s\t%-20s\t%-20s\t%-4d\t%-4d", $sid, $tid, $tmp_many_one_s[$j]->form, $many_to_one_t[$i]->form, $tmp_many_one_s[$j]->ord, $many_to_one_t[$i]->ord);					
				}
				elsif (($i != 0) && ($j == 0)) {
					$out_string = sprintf("%30s\t%40s\t%-20s\t%-20s\t%-4d\t%-4d", '', '', $tmp_many_one_s[$j]->form, $many_to_one_t[$i]->form, $tmp_many_one_s[$j]->ord, $many_to_one_t[$i]->ord);
				}
				else {
					$out_string = sprintf("%30s\t%40s\t%-20s\t%-20s\t%-4d\t%-4s", '', '', $tmp_many_one_s[$j]->form, '', $tmp_many_one_s[$j]->ord, '');
				}
				print { $self->_file_handle } $out_string . "\n";				
			}
		}		
