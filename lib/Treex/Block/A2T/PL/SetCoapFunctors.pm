package Treex::Block::A2T::PL::SetCoapFunctors;
use Moose;
...
sub process_tnode {
    ...
    if ( $t_node->t_lemma =~ /^(a|i|oraz)$/) {
        $functor = "CONJ";
    }
    elsif ( $t_node->t_lemma =~ /^(albo|lub)$/) {
        $functor = "DISJ";
    }
    elsif ( $t_node->t_lemma =~ /^ale$/) {
        $functor = "ADVS";
    }
    elsif ( $afun eq 'Coord' ) {
        $functor = 'CONJ';
...
=item Treex::Block::A2T::PL::SetCoapFunctors
...
phase).
This block is specific for Polish.

=back

=cut

# Copyright 2017, Michal Novák
