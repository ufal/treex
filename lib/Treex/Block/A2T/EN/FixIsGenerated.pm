package Treex::Block::A2T::EN::FixIsGenerated;
use Moose;
use Treex::Moose;
...
sub repair_is_generated
{
    ...
    for my $t_node ( $t_root->get_descendants ) {
        push @{ $a2t{ $t_node->get_attr('a/lex.rf') } }, $t_node if $t_node->get_attr('a/lex.rf') && !$t_node->is_generated;
    }
    for my $a_id ( keys %a2t ) {
        my $list = \@{ $a2t{$a_id} };
        @$list > 1 or next;
        info "Non-generated nodes with a-id $a_id: ", join( ', ', map { $_->get_attr('id') } @$list ), " -- ";
        map { $_->set_is_generated(1) ) } @$list;
...
=item Treex::Block::A2T::EN::FixIsGenerated
