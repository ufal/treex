package SEnglishT_to_TCzechT::Move_genitives_to_postposit;
...
sub process_bundle {
    ...
    foreach my $cs_tnode ( $cs_troot->get_descendants() ) {
        ...
        next if $cs_tnode->get_attr('formeme') !~ /^n:([237]|.*\+\d)/;

        # don't reorder personal pronouns
        next if $cs_tnode->get_attr('t_lemma') eq '#PersPron';
        ...
        next if $en_tnode->get_attr('formeme') !~ /n:(poss|attr)/;
...
=item SEnglishT_to_TCzechT::Move_genitives_to_postposit
