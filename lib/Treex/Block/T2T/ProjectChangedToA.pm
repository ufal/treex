...
sub project {
    ...
    return if $self->is_punct($lex);
    my $src_lex = $src_tnode->get_lex_anode() // return;

    $changed += $self->change($src_lex, $lex);

    # aux nodes
    my %aux = map { ($_->id, $_) } grep { !$self->is_punct($_) } $tnode->get_aux_anodes();
    my %src_aux = map { ($_->id, $_) } grep { !$self->is_punct($_) } $src_tnode->get_aux_anodes();
...
sub change {
    ...
    my $lemma_chaned = !$self->lemmas_eq($anode, $src_anode);
    my $tag_changed = $self->tag_defined($anode) && !$self->tags_eq($anode, $src_anode);
    if ($lemma_chaned || $tag_changed) {
...
sub tag_defined {
    ...
    return (defined $anode->tag);
}

sub lemmas_eq {
    my ($self, $anode1, $anode2) = @_;

    return ($anode1->lemma eq $anode2->lemma);
}

sub tags_eq {
    my ($self, $anode1, $anode2) = @_;

    return ($anode1->tag eq $anode2->tag);
}
...
Treex::Block::T2T::ProjectChangedToA
