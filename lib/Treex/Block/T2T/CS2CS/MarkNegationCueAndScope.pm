...
sub process_ttree {
    ...
    foreach my $neg_tnode (@neg_tnodes_BC) {
        ...
        if (defined $tfa) {
            ...
            if ($self->special eq 'coord' && !grep { $_->is_member || $_->nodetype eq 'coap' } @scope_candidates) {
                next;
            }
