...
my $cog_verbs = 'seem|appear|mean|follow';
...
#         if ( $a_it and $a_it->afun eq "Sb" ) {
            ...
            if ( $t_node->get_lex_anode->lemma eq "it" ) {
                ...
                if ( $aligned and $types->[0] ne "monolingual" ) {
                    $cs_node = $aligned->[0];
                    if ( $cs_node->t_lemma eq "ten" ) {
                        $to_sum++;
                    }
                    elsif ( $cs_node->t_lemma eq "#PersPron" ) {
                        $pp_sum++;
                    }
                }
            }
...
sub has_cs_to {
    my ( $cs_node ) = @_;
    return ( $cs_node->t_lemma eq "to" ) ? 1 : 0;
}

sub has_cs_ACT_clause {
    my ( $en_verb, $cs_verb ) = @_;
    return (
        $en_verb->t_lemma =~ /^(be|s)$/
        and grep { $_->formeme =~ /^v:.*fin$/
    ...
    if ( @descendants < 15 ) {
        foreach my $t_node ( @descendants ) {
            ...
            if ( $a_it ) {
                ...
                if ( ($verb->gram_sempos || "") eq "v" and $a_it->afun eq "Sb" ) {
                    print $t_node->get_address . "\n";
                }
            }
        }
