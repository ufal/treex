...
sub process_atree {
	...
	foreach my $node (@nodes) {
		if ( $node->form =~ /^(.+?)(\p{IsP}+)$/ ) {
			...
			#@aligned_nodes = $node->get_aligned_nodes_of_type('^' . $self->alignment_type . '$', $self->language, $self->selector);
			
#			my @aligned_punctuation_nodes;
#			my @aligned_nonpunct_nodes;
