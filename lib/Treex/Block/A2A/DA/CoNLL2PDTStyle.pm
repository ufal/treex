...
sub detect_coordination
{
    ...
    if(@members)
    {
        ...
        if(scalar(@delimiters)>1 && $delimiters[$#delimiters]->form() eq '"' &&
...
sub collect_coordination_members
{
    ...
    # Minimum requirement: there is the coordinating conjunction among the children (the other member will be attached to the conjunction).
    my @coord = grep {$_->conll_deprel() eq 'coord'} (@children);
    if(@coord)
    {
        my @members0;
        my @delimiters0;
        my @sharedmod0;
        my @privatemod0;
        # OK, this is a coordination root. Then it is the first member, too.
        push(@members0, $croot);
        # Any children of the root tagged 'conj' are middle members of the coordination.
        push(@members0, grep {$_->conll_deprel() eq 'conj'} (@children));
        # The coordinating conjunction should have one child, tagged 'conj', which is the last member.
        # Let's be more general and assume there may be several conjunctions with several members each.
        foreach my $conjunction (@coord)
        {
            push(@members0, grep {$_->conll_deprel() eq 'conj'} ($conjunction->children()));
        }
        # Assume all punctuation attached to the root and to the conjunction(s) are delimiters.
        # So are all the conjunctions.
        foreach my $child (@children)
        {
            if($child->conll_deprel() eq 'pnct')
            {
                push(@delimiters0, $child);
            }
            elsif($child->conll_deprel() eq 'coord')
            {
                push(@delimiters0, $child);
                push(@delimiters0, grep {$_->conll_deprel() eq 'pnct'} ($child->children()));
            }
        }
        # Find the modifiers and sort them out to shared and private.
        $self->collect_coordination_modifiers(\@members0, \@sharedmod0, \@privatemod0);
        # Add the found nodes to the caller's storage place.
        push(@{$members}, @members0);
        push(@{$delimiters}, @delimiters0);
        push(@{$sharedmod}, @sharedmod0);
        push(@{$privatemod}, @privatemod0);
    }
    # If there is no coordinating conjunction but there is punctuation, there may still be coordination members. Example:
    # intolerancen/NN/subj er/VB/vobj i/RR/pred stand/NN/nobj ... -/Z:/pnct ... vil/VB/conj tolerancen/NN/subj sejre/Vf/vobj
    else
    {
        my @members0 = grep {$_->conll_deprel() eq 'conj'} (@children);
        if(@members0)
        {
            unshift(@members0, $croot);
            my @delimiters0 = grep {$_->conll_deprel() eq 'pnct'} (@children);
            # If there is at least one delimiter, we can handle it as coordination.
            if(@delimiters0)
            {
                my @sharedmod0;
                my @privatemod0;
            ...
            # If there are no delimiters, we cannot handle it as coordination.
            # Change afuns of the dependent members so that there is no trace of the coordination.
            else
            {
                foreach my $member (@members0)
                {
                    if($member->afun() eq 'CoordArg')
                    {
                        if($member->parent()->get_iset('pos') =~ m/^(noun|adj|num)$/)
                        {
                            $member->set_afun('Atr');
                        }
                        else
...
sub collect_coordination_modifiers
{
    ...
    {
        my @modifying_children = grep {$_->conll_deprel() !~ m/^(conj|coord|pnct)$/} ($member->children());
        if($member==$croot)
        {
...
#------------------------------------------------------------------------------
# Conjunction as the first word of the sentence is the root of the sentence and
# the root of a deficient coordination whose only member is the main verb. This
# solution is identical in DDT and in PDT. However, because the conjunction's
# deprel is 'ROOT' and not 'coord', our coordination processor will not find
# it. This method corrects it.
#------------------------------------------------------------------------------
sub mark_deficient_clausal_coordination
{
    ...
    if($nodes[0]->match_iset('pos' => 'conj', 'subpos' => 'coor'))
    {
        my @children = $nodes[0]->children();
        if(scalar(@children)==1 && $children[0]->conll_deprel() eq 'conj')
        {
            $nodes[0]->set_afun('Coord');
            $children[0]->set_afun('Pred');
            $children[0]->set_is_member(1);
        }
    }
    # It turns out that the deficient coordination need not necessarily start at the first token of the sentence.
    # Ther may be at least some punctuation if not more:
    # "Men den er...," understreger han.
    # "men" is a coordinating conjunction that is not the first token of the sentence because of the quotation mark.
    # Moreover, its deprel is not coord; it's qobj (quotation object).
    # However, its child, the verb 'er', has the deprel 'conj'.
    # So provided normal coordinations have already been processed, we can now collect all remaining 'CoordArg' nodes.
    # Or at least those whose parent is a coordinating conjunction.
    foreach my $node (@nodes)
    {
        if($node->afun() eq 'CoordArg' && $node->parent()->match_iset('pos' => 'conj', 'subpos' => 'coor'))
        {
