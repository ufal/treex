...
sub _plant_seeds { 
    ...
    foreach (@children) {
        ...
        ) { $_->{seed_type} = 'Apos_member1'; $new_clause = $parent->clause_number; }
        # === embedded ( children ) as seeds (lze spolecne rozviti)
        elsif ( (@apos_start and _is_boundary($apos_start[0]) > 3
