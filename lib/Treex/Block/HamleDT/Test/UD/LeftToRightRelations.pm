...
sub process_anode
{
    ...
    if($deprel =~ m/^(conj|fixed|flat)(:|$)/)
    {
        my $parent = $node->parent();
        if(!defined($parent) || $parent->ord() >= $node->ord())
        {
            $self->complain($node, 'conj|fixed|flat must always go from left to right');
        }
    }
}
...
The relations C<conj>, C<fixed> and C<flat> must always be left-to-right (head-initial).
