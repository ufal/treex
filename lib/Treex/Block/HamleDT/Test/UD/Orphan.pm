            ...
            if(scalar(@arguments)>=2 && scalar(@nconjuncts)>=1)
            {
                ...
                if(scalar(@nconjuncts)>=1)
                {
                    ...
                    foreach my $nc (@nconjuncts)
                    {
                        ...
                        # amod is annotation error but it does not signal orphan (es: Concejo Nacional)
                        my @nchildren = grep {$_->is_noun() && $_->deprel() !~ m/^(orphan|nmod|appos|conj|compound|flat|amod)(:|$)/} ($nc->children());
