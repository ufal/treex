...
sub split_fused_token
{
    ...
    for(my $i = 0; $i <= $#new_nodes; $i++)
    {
        ...
        $nn->_set_ord($ord);
        $nn->wild->{fused_ord} = $ord.'.'.($i+1);
        ###!!! Later we will want to make these attributes normal (not wild).
        $nn->wild->{fused_form} = $fused_node->form();
        $nn->wild->{fused_start} = $new_nodes[0];
        $nn->wild->{fused_end} = $new_nodes[-1];
        $nn->wild->{fused} = ($i == 0) ? 'start' : ($i == $#new_nodes) ? 'end' : 'middle';
    }
