package Treex::Block::HamleDT::HarmonizePerseusIT;
use Moose;
use Treex::Core::Common;
use utf8;
extends 'Treex::Block::HamleDT::HarmonizePDTIT';
...
(
    'ADV'       => 'Adv',
    'APOS'      => 'Apos',# Apos or appos are not valid in HamleDT, so this change is invalid later
    'ATR'       => 'Atr',
    'ATV'       => 'Atv',
    'COORD'     => 'Coord',
    'OBJ'       => 'Obj',
    'OCOMP'     => 'Obj', ###!!!Unsatisfactory solution: we will need to store a wild attribute to remember this information.
...
sub convert_deprels
{
	...
	elsif($deprel=~ s/_AP$//i)
 	{
            $node->set_is_member(1) ;
	    $node->wild()->{apos} = 'apped' ;
    ...
    {
        my $deprel = $node->deprel(); 
        # There are chained dependency labels that describe situation around ellipsis.
    ...
    #!!!_climb_up_below_coap fails when an Ap member is under a co-ordination: it stops earlier than it should: these cases should be distinguished
    foreach my $node (@nodes)
    {
        ...
        if($node->is_member())
        {
            my $new_member = (exists($node->wild()->{apos}) && $node->wild()->{apos} eq 'apped') ? 
						$self->_climb_up_below_ap($node) : $self->_climb_up_below_coap($node);
