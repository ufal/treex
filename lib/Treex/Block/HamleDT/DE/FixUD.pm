...
sub fix_auxiliary_verb
{
    ...
    if($node->tag() eq 'AUX')
    {
        if($node->deprel() =~ m/^aux(:|$)/ && $node->lemma() =~ m/^(bekommen|bleiben)$/)
        {
