package Treex::Block::HamleDT::LV::FixUD;
use utf8;
use Moose;
...
sub identify_acl_relcl
{
    ...
    # relative clause. Example: "kā" ("how").
    return if($subordinator->lemma() =~ m/^(kā)$/);
    # The interrogative-relative adverb "kāpēc" ("why") could be said to corefer with a few
    ...
    return if($subordinator->lemma() eq 'kāpēc' && $parent->lemma() !~ m/^(důvod|příčina|záminka|ten|to)$/);
    ...
    # The interrogative-relative pronouns "kas" ("who") and "ko", "kāda", "kādās", "kāds", "kādos" ("what") usually
    ...
    ###!!! In Latvian, "kas" actually does not to be such a big problem with bad nouns. But at present it does not matter because our list of bad nouns is Czech anyways.
    return if($subordinator->lemma() =~ m/^(kas|ko|kāda|kādās|kāds|kādos)$/ && $parent->lemma() =~ m/^($badnouns)$/);
    ...
    return if($depth > 0 && $subordinator->lemma() =~ m/^(kas|ko|kāda|kādās|kāds|kādos|kura|kurai|kurš)$/ && $subordinator->deprel() !~ m/^(nsubj|obj|iobj|obl|nmod|det)(:|$)/);
    # Relative adverbs: "kur" ("where"), "kad" ("when"), "kā" ("how"), "kāpēc" ("why"), "kādēļ" ("why").
    return if($subordinator->lemma() =~ m/^(kur|kad|kā|kāpēc|kādēļ)$/ && $subordinator->deprel() !~ m/^advmod(:|$)/);
    ###!!! We do not rule out the "bad nouns" for the most widely used relative
    ###!!! word "kura", "kurai", "kurš" ("which"). However, this word can actually occur in
...
=item Treex::Block::HamleDT::LV::FixUD

Latvian-specific post-processing. It can also be used to check for and
