package Treex::Block::HamleDT::HarmonizePDTIT;
use utf8;
use Moose;
...
sub process_zone
{
    ...
    #We remove the following lines because they interact badly with complex co-ordinations, and IT-TB already conforms to the Prague style
    #my $phrase = $builder->build($root);
    #$phrase->project_dependencies(); #!!!! This operation mistakenly changes some Obj deprels to ExDs!!!
...
#------------------------------------------------------------------------------
# As before, but specifically for appositions
#------------------------------------------------------------------------------
sub _climb_up_below_ap
{ 
    my $self = shift;
    my ($node) = @_;
    my $debug_address = $node->get_address();
    ...
    while(1)
    {
        ...
        {
            push(@debug_path, 'ROOT');
            log_warn('No Apos node between a member of apposition and the tree root');
            log_warn($debug_address);
            log_warn('The path: '.join(' ', @debug_path));
            return;
        }
        # We cannot use $parent->is_coap_root() because it queries the afun attribute while we use the deprel attribute.
        elsif ($parent->deprel() =~ m/^Apos/i)
        {
            return $node;
        }
        else
