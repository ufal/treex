...
sub save_tree {
  ...
  #  print $_ ."->".$indices{$_}."\n";
   ...
   $current_sentence[$_]->set_parent($current_sentence[$indices{$_}]);
    }
    }
}
...
#     }
	...
	rank_segment($temp[$max_index]." ".$temp[$max_index-1]);
	rank_segment($temp[$max_index]." ".$temp[$max_index+1]);
	$left=$scores{$temp[$max_index]." ".$temp[$max_index-1]};
	$right=$scores{$temp[$max_index]." ".$temp[$max_index+1]};
	if($left>$right){
	  $direction="LEFT";
	}
      }
  ...
  if($direction eq "RIGHT"){
    $indices{$indices_offset{$max_index}}=$indices_offset{$max_index+1};
  }
