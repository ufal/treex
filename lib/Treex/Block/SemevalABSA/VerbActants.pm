...
sub process_ttree {
    ...
    my @preds = grep { $_->functor eq 'PRED' && is_subjective( $_ ) } $ttree->get_descendants;

    for my $pred (@preds) {
        my $polarity = get_polarity( $pred );
        my @actors = grep { $_->functor eq 'ACT' } get_clause_descendants( $pred );
        my @patients = grep { $_->functor eq 'PAT' } get_clause_descendants( $pred );
        if (@patients) {
            map { mark_node( $_, "verb_actant_pat$polarity" ) } @patients;
        } elsif (@actors) {
            map { mark_node( $_, "verb_actant_act$polarity" ) } @actors;
        }
    }
