...
my @ATTRS_TO_COPY = qw(ord functor is_member);
...
sub process_document {
    ...
    foreach my $bundle ( $document->get_bundles() ) {
        ...
        copy_subtree( $source_root, $target_root, \%used_vars );
        $target_root->set_src_tnode($source_root);
    }
}

sub copy_subtree {
    my ( $source_root, $target_root, $used_vars ) = @_;
    ...
    foreach my $source_node ( $source_root->get_children( { ordered => 1 } ) ) {
        ...
        copy_subtree( $source_node, $target_node, $used_vars );
    }
}
...
sub _create_amr_lemma {
    ...
    my $var_letter = substr $t_lemma, 0, 1;
