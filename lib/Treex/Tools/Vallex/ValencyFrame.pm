package Treex::Tools::Vallex::ValencyFrame;

use Moose;
use Treex::Core::Common;
use MooseX::ClassAttribute;
use XML::LibXML;
use Treex::Tools::Vallex::FrameElement;
...
# The individual frame elements (Treex::Tools::Vallex::FrameElement)
...
around 'BUILDARGS' => sub {
    ...
    if ( ( $params->{id} or $params->{ord} ) and $params->{lexicon} ) {
        ...
        foreach my $element ( $frame_xml->getElementsByTagName('element') ) {
            push( @{ $params->{elements} }, Treex::Tools::Vallex::FrameElement->new( xml => $element, language => $params->{language} ) );
        }
    }
    else {
        $params->{lemma} =~ s/ /_/g;
...
sub _get_xpath_context {
    ...
    if ( !Treex::Tools::Vallex::ValencyFrame->_loaded_dicts->{$lexicon_name} ) {
        my $lexicon = XML::LibXML->load_xml( location => $DEFAULT_LEXICON_PATH . $lexicon_name );
        $lexicon->indexElements();
        Treex::Tools::Vallex::ValencyFrame->_loaded_dicts->{$lexicon_name} = XML::LibXML::XPathContext->new($lexicon);
    }
    return Treex::Tools::Vallex::ValencyFrame->_loaded_dicts->{$lexicon_name};
}
...
Treex::Tools::Vallex::ValencyFrame

=head1 DESCRIPTION

This represents a single valency frame of a tectogrammatical node, containing its lemma, part of speech
and the individual elements (as L<Treex::Tools::Vallex::FrameElement> objects). A valency frame may be created
...
=head1 SYNOPSIS
    ...
    my $frame = Treex::Tools::Vallex::ValencyFrame->new({lemma => 'hlad', pos => 'n', elements => [], language => 'cs' });

    # create a frame from the valency dictionary
    $frame = Treex::Tools::Vallex::ValencyFrame->new( {ord => 3, lexicon => 'vallex.xml', language => 'cs'} );
    $frame = Treex::Tools::Vallex::ValencyFrame->new( {id => 'v-w3f1', lexicon => 'vallex.xml', language => 'cs'} );
    
    # print the frame
    $frame->to_string();

    # access the individual frame elements (Treex::Tools::Vallex::FrameElement)  
...
A direct access to all elements of the valency frame (as an array of L<Treex::Tools::Vallex::FrameElement> objects).

=item C<functor>

    $element = $frame->functor('ACT');
    
This returns a L<Treex::Tools::Vallex::FrameElement> object which represents the element of this valency
...
=item C<new>
    ...
    $frame = Treex::Tools::Vallex::ValencyFrame->new({lemma => 'hlad', pos => 'n', elements => [], language => 'cs' });
    $frame = Treex::Tools::Vallex::ValencyFrame->new( {ord => 3, lexicon => 'vallex.xml', language => 'cs'} );
    $frame = Treex::Tools::Vallex::ValencyFrame->new( {id => 'v-w3f1', lexicon => 'vallex.xml', language => 'cs'} );
