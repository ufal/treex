...
my $DECODING_TABLE = {
    ...
    DA      => { pos => 'adj',  subpos => 'art', definiteness=> 'def'}, # Definite Articles
    ...
    GERAUX  => { pos => 'verb', verbform => 'part', subpos => 'aux'}, # Gerund "ter"/"haver" in compound tenses tendo, havendo
    IA      => { pos => 'adj', subpos => 'art', definitness => 'ind'}, # Indefinite Articles uns, umas, …
    ...
    PNM     => { pos => 'noun', subpos => 'prop'}, # Part of Name    Lisboa, António, João, …
    ...
    PREP    => { pos => 'prep'}, # Prepositions    de, para, em redor de, …
    ...
    UM      => { pos => 'adj', subpos => 'art', definitness => 'ind', numvalue => '1'}, # "um" or "uma"
    UNIT    => { pos => 'noun', abbr => 'abbr'}, # Abbreviated Measurement Unit    kg., km., …
    VAUX    => { pos => 'verb', subpos => 'aux'}, # Finite "ter" or "haver" in compound tenses  temos, haveriam, …
