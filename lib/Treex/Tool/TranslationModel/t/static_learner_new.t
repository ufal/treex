...
sub new_learner_with_data {
    my ($data, $params) = @_;
    my $learner = new_ok( 'Treex::Tool::TranslationModel::Static::RelFreq::Learner_new' => (defined $params ? [ $params ] : []) );

    foreach my $instance (@$data) {
        $learner->see( $instance->{in}, $instance->{out}, undef, $instance->{count} );
    }
    return $learner;
}
...
$learner = new_learner_with_data(\@data, {min_instances => 3});
...
TODO: {
    local $TODO = "unimplemented features of the old static learner";

$model = $learner->get_model({min_pair_count => 2});
ok($model, "learner returns a model");
@transls = $model->get_translations('en1');
is(scalar @transls, 2, 'minimum pair count filtering');

$learner = new_learner_with_data(\@data);
$model = $learner->get_model({max_variants => 2});
ok($model, "learner returns a model");
@transls = $model->get_translations('en1');
is(scalar @transls, 2, 'maximum variants count filtering');

$learner = new_learner_with_data(\@data);
$model = $learner->get_model({min_forward_prob => 0.1});
ok($model, "learner returns a model");
@transls = $model->get_translations('en1');
...
$learner = new_learner_with_data(\@data);
$model = $learner->get_model({min_backward_prob => 1.0});
ok($model, "learner returns a model");
@transls = $model->get_translations('en1');
