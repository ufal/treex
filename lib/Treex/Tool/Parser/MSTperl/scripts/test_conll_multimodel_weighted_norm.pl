...
my ($test_file,  $config_file, $normalization_type, %models) = @ARGV;

my $config = Treex::Tool::Parser::MSTperl::Config->new(config_file => $config_file, normalization_type => $normalization_type);
