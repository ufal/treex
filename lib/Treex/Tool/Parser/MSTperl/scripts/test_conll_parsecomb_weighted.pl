...
my ($test_file, $out_file_suffix, $config_name, $parsed_file_prefix, $parsed_file_suffix, %lang_weights) = @ARGV;
...
my @parsed_sentences = ();
my @weights = ();
foreach my $lang (keys %lang_weights) {
    my $parsed_file = $parsed_file_prefix . $lang . $parsed_file_suffix;
    push @parsed_sentences, $reader->read_tsv($parsed_file);
    push @weights, $lang_weights{$lang};
}
...
foreach my $correct_sentence (@{$test_data}) {
    ...
    my $test_sentence = $parse_combiner->parse_sentence_internal(\@sentence_parses, \@weights);
