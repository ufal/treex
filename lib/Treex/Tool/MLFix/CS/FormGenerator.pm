package Treex::Tool::MLFix::CS::FormGenerator;
use Moose;
use Treex::Core::Common;
use utf8;
extends 'Treex::Tool::MLFix::FormGenerator';

use Treex::Tool::LM::MorphoLM;
use Treex::Tool::Lexicon::Generation::CS;
use Treex::Tool::MLFix::CS::NumberSwitcher;

# TODO: turn these into class attributes
...
has morphoLM => (
	is => 'rw',
	builder => '_build_morphoLM'
);

has numberSwitcher => (
	is => 'rw',
	builder => '_build_numberSwitcher'
);

sub _build_generator {
	my ($self) = @_;
	return Treex::Tool::Lexicon::Generation::CS->new();
}

sub _build_morphoLM {
	my ($self) = @_;
	return Treex::Tool::LM::MorphoLM->new();
}

sub _build_numberSwitcher {
	my ($self) = @_;
    return Treex::Tool::MLFix::CS::NumberSwitcher->new(generator => $self);
}
...
sub get_form {
    ...
    my $form_info = $self->morphoLM->best_form_of_lemma( $lemma, $tag );
    my $form = undef;
    $form = $form_info->get_form() if $form_info;

    if ( !$form ) {
        ($form_info) = $self->generator->forms_of_lemma(
            $lemma, { tag_regex => "^$tag" }
        );
        $form = $form_info->get_form() if $form_info;
    }
...
sub regenerate_node {
    ...
    if ( !$dont_try_switch_number ) {
        $new_tag =
            $self->numberSwitcher->try_switch_node_number( $node, $ennode );
...
Treex::Tool::MLFix::CS::FormGenerator
