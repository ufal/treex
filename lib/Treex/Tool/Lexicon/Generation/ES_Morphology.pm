package Treex::Tool::Lexicon::Generation::ES_Morphology;
use Moose;
...
has dict_name => (is=>'ro', isa=>'Str', default=>'morphodita_model.es');
has dict_path => (is=>'ro', isa=>'Str', lazy_build=>1);
has tool  => (is=>'ro', lazy_build=>1);

sub _build_dict_path {
    my ($self) = @_;
    return Treex::Core::Resource::require_file_from_share('data/models/morphodita/es/'.$self->dict_name);
}
...
Treex::Tool::Lexicon::Generation::ES_Generation
