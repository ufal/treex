...
=head1 SYNOPSIS
    ...
    # 1. Prepare local Perl enviroment, so Perl modules will be installed to ~/perl5
    # If you have a root access and want to install to system paths, skip this step.
    ...
    # Note: Instead of "cpanm" tool, you can use "cpan" if you wish.
    ...
    # The very instalation of Treex::Core is simple but it may take some time
    cpanm Treex::Core
    ...
    # To install Perl Tk module, you may need to install some header file. On Ubuntu/Debian:
    sudo apt-get install libx11-dev libxft-dev libfontconfig1-dev libpng12-dev zlib1g-dev
    # Get a small installation script which automatically downloads and builds everything else.
    ...
    # 5. Download the newest version of the whole Treex from SVN repository (optional)
    svn --username public co https://svn.ms.mff.cuni.cz/svn/tectomt_devel/trunk/treex ~/treex
    # password is "public"
    # Add the following lines to your ~/.bashrc
    export PATH="$HOME/treex/bin:$PATH"
    export PERL5LIB="$HOME/treex/lib:$PERL5LIB"

=head1 0. Prerequisites
...
If you have Perl older than 5.10 (or if you just want to try the newest Perl),
you can install your own Perl using L<perlbrew|http://perlbrew.pl> -- it is really simple.

Note that if you have Windows and I<only> want to browse C<*.treex> files,
...
=head1 1. Prepare local Perl enviroment

In order to install Treex, you must be able to install Perl modules from L<CPAN|http://cpan.org>.
However, this step is not specific to Treex, it is a basic Perl skill.
There are several way how to achieve the goal, but I consider this the easiest one.
If you have already set up your local directory for Perl modules (e.g. using L<local::lib> or C<perlbrew>),
you can just install C<cpanm> by C<cpan App::cpanminus> and skip this step.
If you have a root access and want to install Treex and its dependencies I<for all users to system paths> (C</usr/lib> etc.),
just skip this step (and type C<sudo cpan App::cpanminus>).

    wget -O- http://cpanmin.us | perl - -l ~/perl5 App::cpanminus local::lib

This command downloads and installs two useful tools (Perl modules):
...
L<local::lib> takes care of setting all the enviroment variables needed to install modules without administrative privileges.

Instead of C<wget -0->, you can use C<curl -L> or simply download C<http://cpanmin.us>, save it as C<cpanm> and run
C<perl cpanm -l ~/perl5 App::cpanminus local::lib>.
Instead of C<~/perl5>, you can use any path you like, but C<~/perl5> is a common standard.
...
The first line sets up the enviroment variables C<$PERL5LIB>, C<$PATH>, C<$PERL_MB_OPT> etc. for the current shell session.
So you can use the modules installed in C<~/perl5> (without specifying this path using C<perl -I>)
and also it ensures that new modules will be installed (using C<cpanm> or C<cpan>) to C<~/perl5> (not to the system paths).
The second line ensures that this setting will be applied also in other (non-login) shell sessions.
The third line ensures that this setting will be applied also in "login" shell sessions (e.g. when you log in via C<ssh>).

=head1 2. Install Treex::Core and its dependencies from CPAN
...
If it fails and C<~/.cpanm/build.log> contains "looking for -lxml2... no",
you are probably missing the header files or the whole library.
On Ubuntu/Debian you can install it with:

    sudo apt-get install libxml2-dev zlib1g-dev

If you know a simple way how to do this without admin privilegies, let me know.
On other systems (e.g. RPM based), try to find similarly named packages (libxml2-devel),
or look at L<http://xmlsoft.org>.
After succesfully installing C<XML::LibXML>,
the installation of L<Treex::Core> should be smooth (but it takes more than 8 minutes if no dependecies are installed):

    cpanm Treex::Core

Rarely, you may encounter problems with installing some modules.
In that case, you should find the first module where the something went wrong.
...
C<treex> is the main Treex script. C<treex -h> should just print the usage info and exit.
The usage will be described in later on in this tutorial (L<Treex::Tutorial::FirstSteps>),
running the command serves here only as a check that C<treex> was installed and can be found in the C<$PATH>.
The installation created a configuration file C<~/.treex/config.yaml> which will be described in L<Treex::Tutorial::Config>.

=head1 3. Install Treex modules for processing English
...
<TrEd|http://ufal.mff.cuni.cz/tred> is a fully customizable and programmable graphical editor and viewer for tree-like structures.
...
To automatically download and build the latest TrEd and its dependencies to C<~/tred>, use:

    bash install_tred.bash --tred-dir ~/tred

You can run C<~/tred/bin/start_tred> to check the GUI.
Treex Core contains an L<extension|Treex::Core::TredView> for TrEd,
so it can open C<*.treex>, C<*.treex.gz> and C<*.streex> files and use the Treex stylesheet.
...
TrEd installed several its dependencies to C<~/tred/dependencies>,
but we want make them permanently available for Treex (and all Perl modules):
