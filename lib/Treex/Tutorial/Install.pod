...
If C<env | grep PERL> prints something,
there is a risk that your previously installed local Perl environment is not set up properly.
See L<step 1|/1. Prepare local Perl environment> for details.
...
See L<step 2|/2. Install Treex::Core from CPAN> for details.
...
If you have already B<properly> set up your local directory for Perl modules
(e.g. using L<local::lib> or L<perlbrew|http://perlbrew.pl>),
you can just skip this step (and possibly install C<cpanm> by C<cpan App::cpanminus>).
However, there is a possibility that your previously installed local Perl environment
is set up only partially and the procedure described here may fail
(e.g. with "I<WHOA THERE! It looks like you've got INSTALL_BASE=...>" in F<~/.cpanm/build.log>).
Command C<env | grep PERL> should print all Perl-related environment variables.
If nothing is printed you can start with the installation procedure.
Otherwise, if you do not need/want your previous local Perl environment,
you can delete (comment out) the listed variables in your shell profile (F<~/.bashrc>, F<~/.profile> etc).
Otherwise, make sure that you have set up I<all> the needed variables,
so you have in your F<~/.bashrc> something like:

 export PERL5LIB=$HOME/perl5/lib/perl5/i386-linux:$HOME/perl5
 export PERL_MB_OPT="--install_base $HOME/perl5"
 export PERL_MM_OPT="INSTALL_BASE=$HOME/perl5"
 export PATH="$HOME/perl5/bin:$PATH"

You should substitute I<i386-linux> with your actual architecture,
which can be detected with C<perl -MConfig -E 'say $Config{archname}'>.
