...
=item 1. Prepare your local Perl enviroment, so Perl modules will be installed to F<~/perl5>.
...
Note: Instead of L<cpanm> tool, you can use L<cpan> if you wish.
...
If you have a root access and want to install Treex and its dependencies I<for all users to system paths> (F</usr/lib> etc.),
...
Instead of C<wget -0->, you can use C<curl -L> or simply download L<http://cpanmin.us>, save it as F<cpanm> and run
C<perl cpanm -l ~/perl5 App::cpanminus local::lib>.
Instead of F<~/perl5>, you can use any path you like, but F<~/perl5> is a common standard.
...
So you can use the modules installed in F<~/perl5> (without specifying this path using C<perl -I>)
...
You can run F<~/tred/bin/start_tred> to check the GUI.
Treex Core contains an L<extension|Treex::Core::TredView> for TrEd,
so it can open F<*.treex>, F<*.treex.gz> and F<*.streex> files and use the Treex stylesheet.
...
TrEd installed several its dependencies to F<~/tred/dependencies>,
...
Finally, you need to put the path to the downloaded modules to your C<$PERL5LIB>.
...
Some of the modules may have dependecies that you do not have (installed).
